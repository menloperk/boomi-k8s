...
usage() {
	...
	-h <proxy host> -e <proxy username> -d <proxy password> -r <proxy port> -k <installation token> \n\n
	Add a Cloud node\n$0 -n <cloud name> -b <boomi container name> -s <node suffix> -i <installation directory> -f <docker uid> \n\n " 1>&2; exit 1;
}
...
while getopts "u:p:n:a:c:s:b:t:i:l:m:y:f:h:e:r:d:k:" o; do
    case "${o}" in
        f)
            dockeruid=${OPTARG}
            ;;
